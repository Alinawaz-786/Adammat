...
interface IWithOutTaxFee{
    function transferWithOutTaxFee( address recipient, uint256 amount) external returns (bool);
}
interface IERC20 is IWithOutTaxFee {
...
contract Adamant is Context, IERC20, Ownable {
    ...
    event ClaimReward(address indexed from, address _recipient,uint256 _amount);
    ...
    function transferWithOutTaxFee(address _recipient, uint256 _amount) public override returns (bool) {
    ...
    function claimRewards(address _recipient,uint256 _amount) public onlyOwner() {
        require(_amount > 0, "Amount must be greater then 0");
        _transfer(_msgSender(), _recipient, _amount);
        emit ClaimReward(_msgSender(), _recipient, _amount);
    }
